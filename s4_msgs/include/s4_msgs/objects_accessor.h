...
public:
  ...
  bool SetFocus(const s4_msgs::TrackedInfo& info);
  ...
  s4_msgs::TrackedObjectArray getList(void)const;
...
bool ObjectsAccessor::SetFocus(const s4_msgs::TrackedInfo& info){
  focus_info_ = info;
  return true;
}
...
bool ObjectsAccessor::SetNearest(geometry_msgs::Point refferece_point){
  ...
  for(int i =0; i < focus_objects.objects.size(); i++){
...
s4_msgs::TrackedObjectArray ObjectsAccessor::getList(void)const{
  return objects_;
}

int ObjectsAccessor::getFocusIndex(void)const{
  int index = -1;
  for(int i = 0; i < (int)objects_.objects.size(); i++){
    if(objects_.objects[i].info.category == focus_info_.category){
      if(objects_.objects[i].info.id == focus_info_.id){
        index = i;
        break;
