...
void seriallink_callback(const s4_comport::SerialCode& serialcode_msg){
	if(serialcode_msg.command[0]=="CANLINK"){
		printf("CANLINK_conv\n");
		s4_comport::CANCode outdata;
...
{
	ros::init(argc, argv, "s4_comport_seriallink_to_canlink");
	ros::NodeHandle n;
	//publicher
	canlink_pub = n.advertise<s4_comport::CANCode>("CANLink_in", 1000);
	//Subscriber
