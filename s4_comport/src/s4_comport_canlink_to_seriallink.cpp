...
#include "s4_comport/SerialCode.h"

#include <sstream>
#include <string>

int cancode_to_serialcode(s4_comport::SerialCode *serialcode, s4_comport::CANCode cancode){
	char tmp_data[2];
	s4_comport::SerialCode midcode;
...
void canlink_callback(const s4_comport::CANCode& cancode_msg){
	s4_comport::SerialCode outdata;
...
{
	ros::init(argc, argv, "s4_comport_canlink_to_seriallink");
	ros::NodeHandle n;
	//publicher
	seriallink_pub = n.advertise<s4_comport::SerialCode>("SerialLink_out", 1000);
	//Subscriber
