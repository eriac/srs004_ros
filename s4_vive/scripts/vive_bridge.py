    ...
    while not rospy.is_shutdown():
        ...
        for deviceName in v.devices:
            ...
            if "LHR" in v.devices[deviceName].get_serial():
                [qx, qy, qz, qw] = tf.transformations.quaternion_multiply([qx, qy, qz, qw], tf.transformations.quaternion_from_euler(math.pi, math.pi/2.0, -math.pi/2.0))
            ...
            if "reference" not in deviceName:
                if deviceName + "_odom" not in publisher:
                    ...
                    #publisher[deviceName + "_odom"] = rospy.Publisher(publish_name_str + "/odom", Odometry, queue_size=50)
                    #publisher[deviceName + "_inverted_odom"] = rospy.Publisher(publish_name_str + "/inverted_odom", Odometry, queue_size=50)
                    publisher[deviceName + "_pose"] = rospy.Publisher(publish_name_str + "/pose",PoseWithCovarianceStamped,queue_size=10)
                    publisher[deviceName + "_joy"] = rospy.Publisher(publish_name_str + "/joy",Joy,queue_size=10)
                ...
                #joy
                joy=Joy()
                joy.header.stamp = time
                state=v.devices[deviceName].get_state()
                joy.axes=[0.0,0.0,0.0]
                joy.axes[0]=state.rAxis[0].x
                joy.axes[1]=state.rAxis[0].y
                joy.axes[2]=state.rAxis[1].x
                joy.buttons=[0,0,0,0]
                if state.ulButtonPressed&1<<33:
                    joy.buttons[0]=2
                elif state.ulButtonTouched&1<<33:
                    joy.buttons[0]=1
                else:
                    joy.buttons[0]=0
                if state.ulButtonPressed&1<<32:
                    joy.buttons[1]=2
                elif state.ulButtonTouched&1<<32:
                    joy.buttons[1]=1
                else:
                    joy.buttons[1]=0
                if state.ulButtonPressed&1<<1:
                    joy.buttons[2]=2
                else:
                    joy.buttons[2]=0
                if state.ulButtonPressed&1<<2:
                    joy.buttons[3]=2
                else:
                    joy.buttons[3]=0
                ...
                if not (x == 0.0 and y == 0.0 and z == 0.0 and vx == 0.0 and vy == 0.0 and vz == 0.0 and v_roll == 0.0 and v_pitch == 0.0 and v_yaw == 0.0):
                    ...
                    publisher[deviceName + "_joy"].publish(joy)
